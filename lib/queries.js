...
/**
 * Get the information of the KBO organization linked to the given administrative unit.
 * @param {string} administrativeUnit The URI of the administrative unit.
 * @returns {Promise<{kboOrg?: string, abbUnit?: string, changeTime?: string, contactPointUri?: string, addressUri?: string}>} The information of the KBO organization.
 */
...
export async function createNewKboOrg(kboOrg, kboId, kboStructuredId) {
  ...
  if (kboOrg.changeTime) {
    updateStr += `; <http://purl.org/dc/terms/modified> ${sparqlEscapeString(
      kboOrg.changeTime
    )}`;
  }
  if (kboId) {
